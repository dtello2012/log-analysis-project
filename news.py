...
# from newsdb import get_popular_articles, get_popular_authors, get_most_error_day
...
.popular-section .articles-img.books:after {
  content: "";
  position: absolute;
  background-image: url("https://www.mysafetytrainingonline.com/wp-content/uploads/2017/09/Ibooks.png");
  width: 50px;
  height: 50px;
}
...
.popular-section .articles-img.authors:after {
  ...
  background-image: url("https://allauthor.com/images/author-icon.png");
  width: 40px;
  height: 40px;
}
...
.popular-section .articles-img.error-log:after {
  content: "";
  position: absolute;
  background-image: url("https://errornetwork.com/files/2017/09/Error-128.png");
  width: 30px;
  height: 30px;
  top: 8px;
}
...
nav .main-nav {
  ...
  <body>
  <header class="page-static-header">
...
def get_popular_article_titles():
    popular_articles_titles_ = "".join(POPULAR_ARTICLES  % (title, views) for title, views in get_code_items(query1))
    return popular_articles_titles_

def get_popular_authors_names():
    popular_authors_names = "".join(POPULAR_AUTHORS  % (name, views) for name, views in get_code_items(query2))
    return popular_authors_names

def geterror_and_percent():
    popular_authors_names = "".join(POPULAR_ERROR_DATE  % (date, percent) for date, percent in get_code_items(query3))
    return popular_authors_names   
